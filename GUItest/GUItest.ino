...
volatile bool timetoupdate = false; //set to high when it is time to draw
uint8_t ledmode = 1; //default is builtin LED mode
...
uint8_t intreadingsmem[8][18]; //stores up to 8 historical readings
...
void handleBTNINT(){
  ...
  if(sensemode == 1){ //continuous, don't know how to implement yet
    measure(ledmode);
  }
  else{ //burst and then average, can be from 2-9 readings
    for(uint8_t i = 0; i < sensemode; i++){
      measure(ledmode);
      for(uint8_t j = 0; j < 18; i++){
        intreadingsmem[i][j] = intreadings[j];
      }
    }
    //average these
  }
